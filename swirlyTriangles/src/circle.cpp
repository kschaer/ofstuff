...
void mountain::draw(){
    ...
    for (int i = 0; i <20; i++){
        for (int j = 0; j < 20; j++){
            ...
            colMod = ofMap(i*sin(ofGetElapsedTimef()), -20,20,0,255);
            ...
            color.set(255-colMod/4,255-colMod/2,120+j*6);
            
            ofSetColor(color);
            
            
            ofDrawTriangle(posL.x,posL.y,posR.x,posR.y,peak.x,peak.y);
            color.set(20*i+10*j, 255-20*j, 20*i);
            ofSetColor(color);
            ofDrawTriangle(posR.x-10, posL.y, posR.x, posR.y, peak.x, peak.y);
            ofSetColor(255);
            ofDrawCircle(peak.x, peak.y, i);
